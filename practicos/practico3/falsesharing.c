...
False sharing
=============
El siguiente código implementa una aproximación de pi usando un método de Monte
Carlo. El código ha sido paralelizado, pero no escala.
...
static double pi(void) {
	...
	#pragma omp parallel default(shared) private(x,y,tid) reduction(+:hits)
	{
		...
		// evita releer la linea de cache que es modificada por cada hilo
		// cada vez que se obtiene un nuevo numero aleatorio
